...
/**
 * 신입 화면 두 개(챗봇·체크리스트)와 앱 셸이 함께 쓰는 배정 정보.
 *
 ...
 *
 * - 셸(AppShell)     : mentorName — 헤더의 "사수 김사수"
 * - 챗봇(ChatPage)   : document_id — 출처 칩에 붙일 챕터 목록 조회에 필요
 * - 체크리스트       : mentorName — "사수 김사수 님이 작성했습니다"
 *
...
const NewcomerScopeContext = createContext(EMPTY);
...
function NewcomerScope({ children }) {
  ...
  return (
    <NewcomerScopeContext.Provider value={scope}>{children}</NewcomerScopeContext.Provider>
  );
}
