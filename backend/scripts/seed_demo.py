...
# logs: (며칠 전, 시, 역할 or None(답을 못 찾은 질문), 질문 유형)

CHECKLIST_SHAPE = [("read", "S11"), ("do", "S11"), ("read", "S21"), ("do", "S21"), ("read", "S31")]

ARCHETYPES = {
    # 순조로운 적응 — 여러 업무를 고르게 묻고, 완료한 업무는 다시 묻지 않고, 뒤로 갈수록 판단 질문
    "steady": {
        "assigned": 21,
        "done": [18, 14, 9, None, None],
        "logs": [
            (20, 10, "S11", "fact"), (19, 14, "S11", "procedure"), (18, 11, "T1", "fact"),
            (16, 15, "S12", "procedure"), (13, 10, "S21", "procedure"), (12, 16, "S22", "judgment"),
            (10, 11, "S21", "judgment"), (8, 14, "S31", "judgment"), (6, 10, "S22", "judgment"),
            (5, 15, "S32", "procedure"), (3, 11, "S12", "judgment"), (2, 16, "T2", "advanced"),
        ],
    },
    ...
    "repeat": {
        ...
        "logs": [
            (17, 10, "S11", "fact"), (16, 14, "S11", "fact"), (15, 11, "T1", "procedure"),
            (11, 10, "S11", "procedure"), (10, 9, "S11", "fact"), (9, 15, "S21", "procedure"),
            (8, 11, "S11", "procedure"), (7, 11, "S12", "fact"), (6, 14, "S21", "judgment"),
            (5, 10, "S11", "procedure"), (4, 16, "S21", "procedure"), (3, 11, "S22", "judgment"),
            (2, 10, "S11", "procedure"), (1, 15, "S32", "judgment"),
        ],
    },
    # 후반부 질문 급감 — 기간 앞쪽에는 활발히 묻다가 뒤쪽 절반에는 거의 묻지 않는다
    "silence": {
        "assigned": 16,
        "done": [14, 10, 6, None, None],
        "logs": [
            (15, 10, "T3", "fact"), (14, 15, "S31", "fact"), (13, 11, "T1", "fact"),
            (12, 14, "S12", "procedure"), (11, 10, "S32", "fact"), (10, 16, "S21", "procedure"),
            (9, 10, "S22", "judgment"), (9, 15, "S21", "judgment"), (9, 20, None, "fact"),
            (2, 11, "T1", "fact"),
        ],
    },
    # 질문이 깊어지는 성장 — 초반 사실 확인에서 판단·심화 질문으로 옮겨간다
    "growth": {
        "assigned": 20,
        "done": [17, 12, 8, None, None],
        "logs": [
            (19, 10, "S11", "fact"), (18, 14, "T1", "procedure"), (17, 11, "S11", "procedure"),
            (15, 15, "S12", "judgment"), (13, 10, "S21", "judgment"), (11, 16, "S22", "judgment"),
            (10, 11, "S21", "judgment"), (7, 14, "S31", "advanced"), (6, 10, "S22", "advanced"),
            (4, 15, "S32", "advanced"), (3, 11, "S12", "judgment"), (1, 16, "T3", "advanced"),
        ],
    },
    # 배정 1주차 — 아직 판단하기 이른 단계
    "early": {
        "assigned": 5,
        "done": [3, None, None, None, None],
        "logs": [(4, 10, "S11", "fact"), (4, 15, "T1", "fact"), (2, 11, "S12", "fact"), (1, 14, "S21", "fact")],
    },
    # 한 영역에만 질문이 몰림 — 둘째 대분류 안에서만 묻는다
    "narrow": {
        "assigned": 15,
        "done": [12, 8, None, None, None],
        "logs": [
            (14, 10, "S21", "fact"), (13, 15, "S21", "fact"), (12, 11, "S22", "procedure"),
            (10, 14, "S21", "procedure"), (9, 10, "T2", "fact"), (7, 16, "S22", "procedure"),
            (5, 11, "S21", "judgment"), (3, 15, "S22", "procedure"), (2, 10, "S21", "procedure"),
        ],
    },
}
...
COMPANIES = [
    ...
    },
    {
        ...
        "questions": {
            "T1": {"fact": ["배차는 몇 시 물량 기준이에요?"], "procedure": ["오후에 들어온 물량은 어떻게 배차해요?"]},
            "S11": {
            ...
            "S22": {"procedure": ["지연 안내는 언제 연락해요?", "예상 도착 시간은 어떻게 계산해요?", "지연 쿠폰은 어디서 발급해요?"],
        ...
        "questions": {
            ...
            "S11": {"fact": ["스테이징에서 얼마나 기다렸다가 배포해요?", "배포 후 얼마나 지켜봐요?", "deploy-prod는 누가 실행해요?"],
                    "procedure": ["운영 배포는 어떻게 시작하나요?", "배포 채널에 PR 목록은 어떻게 적어요?",
                                  ...
                                  "배포 순서를 다시 한 번 알려주세요"]},
            "S12": {"fact": ["롤백 기준이 뭐예요?"], "procedure": ["롤백은 어떻게 해요?"],
            ...
            "S22": {"fact": ["알림은 어떤 기준으로 와요?"],
                    "procedure": ["야간 알림은 어떻게 받아요?", "5xx 비율 알림이 오면 어디부터 확인해요?",
                                  "알림 기준 값은 어디서 확인해요?"],
                    "judgment": ["p95가 0.9초에서 오래 머물면 미리 알려야 하나요?"],
                    "advanced": ["알림 기준을 서비스별로 다르게 둘 수 있을까요?"]},
            "T3": {"fact": ["로컬에서 DB는 어떻게 띄워요?"], "advanced": ["개발 환경 구성을 문서로 자동화해도 될까요?"]},
            "S31": {"fact": ["시드 데이터는 어떻게 넣어요?"], "judgment": ["운영 데이터 일부만 로컬로 받아도 되나요?"]},
    ...
    },
    {
        ...
        "questions": {
            ...
            "S12": {"fact": ["면접관은 몇 명이 들어가요?"], "procedure": ["면접 일정은 언제까지 안내해요?"],
            ...
            "S22": {"procedure": ["급여 문의는 어디로 받아요?", "계산 오류 차액은 어떻게 지급해요?", "급여 명세서 재발급은 어떻게 해요?"],
                    "judgment": ["메신저로 급여 문의가 오면 어떻게 답해요?", "오류 금액이 작으면 다음 달에 반영해도 되나요?"],
                    "advanced": ["자주 오는 급여 문의를 FAQ로 만들어도 될까요?"]},
            "T3": {"fact": ["근태 관리는 어떤 규정을 따라요?"], "advanced": ["노무 이슈가 생기면 외부 노무사와 언제 상의해요?"]},
...
def build_plan(spec):
    ...
    for index, (name, archetype_key) in enumerate(spec["newcomers"]):
        ...
        logs = []
        for days_ago, hour, role, qtype in arch["logs"]:
            if role is None:
                logs.append({"created_at": at(days_ago, hour), "chapter_title": None, "question_type": qtype,
                             "question": "사내 복지 포인트는 어디서 신청해요?", "answer": NO_ANSWER, "answered": False})
                continue
            ch = by_role[role]
