...
ARCHETYPES = {
    ...
    "steady": {
        ...
        "done": [18, 14, 9, 4, None],
        "phases": [
            # 1주차: 문서를 처음 훑으며 기본을 몰아서 묻는다
            phase(20, 15, [6, 4, 5, 3, 5], ["S11", "S12", "S11", "T1", "S12"], ["fact", "fact", "procedure"]),
            # 2주차: 실제로 해보며 절차·판단 질문으로 옮겨간다
            phase(14, 8, [3, 5, 2, 4, 0, 3], ["S21", "S22", "S21", "T2", "S22"], ["judgment", "procedure", "judgment"]),
            # 3주차: 손에 익어 질문이 줄고, 남은 업무만 띄엄띄엄 묻는다
            phase(7, 1, [2, 0, 3, 1, 2], ["S31", "S32", "S22", "T3", "S12"], ["judgment", "advanced", "judgment"]),
        ],
    },
    # 완료한 업무를 반복해서 물음 — 읽고 실습까지 체크한 업무를 이후에도 계속 묻는다
    "repeat": {
        # 같은 걸 또 묻는 유형이라 질문 문장도 돌려 쓴다 (recycle)
        "recycle": True,
        "assigned": 18,
        "done": [16, 13, 12, None, None],
        "phases": [
            phase(17, 14, [6, 4, 5, 3], ["S11", "S11", "T1", "S12"], ["fact", "fact", "procedure"]),
            phase(13, 8, [4, 2, 5, 3, 0], ["S11", "S21", "S11", "S22"], ["procedure", "fact", "procedure"]),
            # 완료 체크한 업무를 끝까지 다시 묻는다 — 이해 일치도가 떨어지는 구간
            phase(7, 1, [3, 5, 2, 4, 1], ["S11", "S21", "S11", "S22"], ["procedure", "procedure", "judgment"]),
        ],
    },
    ...
    "silence": {
        ...
        "phases": [
            # 처음 나흘은 하루 대여섯 건씩 활발하다가, 그 뒤로 뚝 끊긴다
            phase(17, 14, [7, 5, 8, 4], ["T3", "S31", "T1", "S12", "S32", "S21", "S22"], ["fact", "fact", "procedure"], miss=2),
        ],
    },
    ...
    "growth": {
        ...
        "done": [17, 12, 8, 3, None],
        "phases": [
            phase(19, 15, [6, 3, 5, 4], ["S11", "T1", "S11", "S12"], ["fact", "fact", "procedure"]),
            phase(14, 8, [4, 2, 5, 0, 3], ["S21", "S12", "S22", "S21"], ["judgment", "procedure", "judgment"]),
            phase(7, 1, [3, 2, 4, 1, 2], ["S22", "S31", "S32", "T3", "S12"], ["advanced", "judgment", "advanced"]),
        ],
    },
    ...
    "early": {
        ...
        "phases": [phase(4, 1, [7, 4, 6, 5], ["S11", "T1", "S11", "S12", "S21"], ["fact", "fact", "procedure"])],
    },
    ...
    "narrow": {
        ...
        "phases": [
            phase(14, 8, [5, 3, 0, 6, 2], ["S21", "S21", "S22", "T2"], ["fact", "procedure", "procedure"]),
            phase(7, 1, [3, 6, 1, 2, 4], ["S21", "S22", "S21"], ["procedure", "judgment", "procedure"], miss=2),
        ],
    },
}
...
COMPANIES = [
    {
        ...
        "questions": {
            "T1": {"fact": ["등록하고 바로 판매되나요?", "상품 등록은 하루에 몇 건까지 해요?",
                           "검수는 보통 얼마나 걸려요?", "등록 담당이 따로 있나요?"],
                   "procedure": ["행사 상품은 언제까지 등록해야 해요?", "급한 상품은 어떻게 먼저 올려요?"]},
            "S11": {
                "fact": ["상품 등록은 어디서 하나요?", "대표 이미지 크기 기준이 있나요?", "검수 반려 사유는 어디서 봐요?",
                         "상세페이지 양식이 정해져 있나요?", "등록 후 수정은 언제까지 가능해요?",
                         "임시 저장한 상품은 어디서 찾아요?", "옵션은 최대 몇 개까지 넣어요?"],
            ...
            "S12": {"fact": ["가격을 바꾸면 바로 반영되나요?", "판매 중지하면 주문은 어떻게 돼요?",
                             "할인가는 어디서 설정해요?", "품절 표시는 자동으로 바뀌나요?",
                             "수정 이력은 어디서 확인해요?"],
            ...
            "S21": {"fact": ["정산 마감이 언제예요?", "정산 예정표는 언제 와요?"],
                    "procedure": ["정산 예정표 금액이 다르면 어떻게 이의를 올려요?", "마감 이후 취소된 건은 어떻게 처리돼요?",
                                  ...
                                  "수수료는 어디서 확인해요?", "정산 보류 건은 어떻게 풀어요?",
                                  "세금계산서는 언제 발행돼요?", "정산 내역을 월별로 비교하려면요?",
                                  "누락된 주문은 어떻게 추가해요?"],
            ...
            "S22": {"fact": ["정산 예외 시트는 어디에 있어요?"],
                    ...
                    "judgment": ["마감 이틀 전에 예외 요청이 오면 받아도 되나요?", "예외 건이 많으면 먼저 공유해야 하나요?",
                                 "예외 처리 금액이 크면 누구 확인을 받아요?", "고객 귀책인데 예외로 넣어도 되나요?",
                                 "같은 주문이 두 번 예외로 올라오면 어떻게 해요?"],
    ...
    },
    {
        ...
        "questions": {
            "T1": {"fact": ["배차는 몇 시 물량 기준이에요?", "하루 평균 물량이 얼마나 돼요?",
                           "기사님은 몇 분이나 계세요?", "배차는 누가 최종 확인해요?"],
            ...
            "S12": {"fact": ["예비 차량은 몇 대예요?", "용차 비용은 어디서 확인해요?", "예비 차량은 누가 배정해요?"],
                    "procedure": ["용차는 어떻게 요청해요?", "긴급 배차는 어느 채널로 알려요?"]},
            ...
            "S21": {"fact": ["파손 보고가 들어오면 제일 먼저 뭘 확인해요?", "파손 사고 보험 기준 금액이 얼마예요?"],
                    "procedure": ["파손 보고 들어오면 뭐부터 해요?", "재발송하기로 하면 어디로 넘겨요?",
                                  ...
                                  "고객 보상은 누가 결정해요?", "파손 상품 회수는 어떻게 요청해요?",
                                  "사고 접수 번호는 어디서 확인해요?", "기사님 진술은 어디에 남겨요?",
                                  "보상 처리 결과는 고객에게 어떻게 알려요?"],
            ...
            "S22": {"fact": ["지연 기준 시간이 몇 시간이에요?"],
                    ...
                    "judgment": ["2시간 반 늦어지는데 쿠폰 줘야 해요?", "기사 연락이 안 되면 고객에게 뭐라고 안내해요?",
                                 "고객이 재배송 대신 환불을 원하면 어떻게 해요?", "지연이 반복되는 권역은 따로 보고해요?",
                                 "날씨 때문에 늦으면 안내 문구를 바꿔야 하나요?"]},
    ...
    },
    {
        ...
        "questions": {
            "T1": {"fact": ["배포는 무슨 요일에 해요?", "이번 주 배포 목요일 맞죠?", "배포 시간은 몇 시예요?",
                           "배포는 누가 승인해요?"],
                   "procedure": ["배포 전에 채널에 뭘 적어요?", "배포 일정은 어디서 확인해요?"]},
            "S11": {"fact": ["스테이징에서 얼마나 기다렸다가 배포해요?", "배포 후 얼마나 지켜봐요?", "deploy-prod는 누가 실행해요?",
                         "배포 기록은 어디에 남아요?", "스테이징 주소가 어떻게 돼요?", "배포에 보통 얼마나 걸려요?"],
            ...
            "S21": {"fact": ["장애 등급은 어떻게 나눠요?", "회고 문서는 며칠 안에 써야 해요?"],
                    "procedure": ["알림 오면 제일 먼저 뭐 해요?", "장애 끝나면 회고는 언제까지 써요?", "장애 채널에는 뭐라고 남겨요?",
                                  ...
                                  "장애 공지는 어느 채널에 올려요?", "로그는 어디서 봐요?", "대시보드 접근은 어떻게 해요?",
                                  "장애 등급은 누가 정해요?", "재발 방지 항목은 어디에 적어요?",
                                  "온콜 인수인계는 어떻게 해요?"],
            ...
            "S22": {"fact": ["알림은 어떤 기준으로 와요?", "야간 호출 기준이 어떻게 돼요?"],
                    ...
                    "judgment": ["p95가 0.9초에서 오래 머물면 미리 알려야 하나요?",
                                 ...
                                 "새벽에 오는 알림은 아침에 봐도 될까요?", "같은 알림이 여러 번 오면 하나로 묶어도 되나요?",
                                 "테스트 중에 뜬 알림도 기록해야 하나요?"],
                    "advanced": ["알림 기준을 서비스별로 다르게 둘 수 있을까요?"]},
            "T3": {"fact": ["로컬에서 DB는 어떻게 띄워요?", "개발 환경 준비는 언제까지 끝내야 해요?"],
                   "advanced": ["개발 환경 구성을 문서로 자동화해도 될까요?"]},
            "S31": {"fact": ["시드 데이터는 어떻게 넣어요?", "로컬 포트는 몇 번을 써요?", "환경 변수는 어디서 받아요?"],
    ...
    },
    {
        ...
        "questions": {
            "T1": {"fact": ["채용은 어디서부터 인사팀이 맡아요?", "채용은 보통 몇 주 걸려요?",
                           "지원자 정보는 어디서 봐요?", "채용 담당이 부서별로 나뉘어 있나요?"],
                   "procedure": ["현업 요청이 들어오면 제일 먼저 뭘 해요?", "인력 요청서는 어디서 받아요?"]},
            "S11": {"fact": ["공고는 어디에 올려요?", "공고 마감일은 며칠 뒤로 잡아요?", "인력 요청서는 누가 승인해요?",
                         "공고 조회수는 어디서 확인해요?", "채용 사이트 계정은 누가 관리해요?",
                         "공고 템플릿이 따로 있나요?"],
            ...
            "S21": {"fact": ["근태 마감은 며칠이에요?", "급여 대장은 며칠에 확정해요?"],
                    "procedure": ["급여 대장은 누구 확인을 받아요?", "20일 이후 수당 변경은 어떻게 처리해요?",
                                  ...
                                  "4대보험 공제는 어디서 확인해요?", "연장근무 수당은 어떻게 계산해요?",
                                  "급여 이체는 누가 실행해요?", "명세서는 언제 발송돼요?"],
            ...
            "S22": {"fact": ["급여 문의는 보통 며칠에 몰려요?"],
                    ...
                    "judgment": ["메신저로 급여 문의가 오면 어떻게 답해요?", "오류 금액이 작으면 다음 달에 반영해도 되나요?",
                                 "본인이 아닌 사람이 급여를 물어보면 어떻게 하죠?", "퇴사자가 급여를 문의하면 어떻게 해요?",
                                 "같은 문의가 계속 오면 공지로 올려도 될까요?"],
    ...
    for ph in arch["phases"]:
        ...
        for days_ago in range(ph["start"], ph["end"] - 1, -1):
            ...
            for i in range(per_day):
                ...
                mix_cycle += 1
                ch = by_role[role]
                question, qtype = take(role, qtype)
                logs.append({
...
def build_plan(spec):
    ...
    for index, (name, archetype_key) in enumerate(spec["newcomers"]):
        ...
        recycle = arch.get("recycle", False)
        used = {}

        def take(role, qtype, pools=pools):
            """그 업무·유형의 질문 문장 하나. 반환은 (질문, 실제 유형)."""
            pool = pools[role]
            if pool.get(qtype):
                question = pool[qtype].pop(0)
                used.setdefault((role, qtype), []).append(question)
                return question, qtype
            if recycle and used.get((role, qtype)):
                # 앞에서 했던 질문을 그대로 다시 한다 — 되묻는 신입의 실제 모습이다
                asked = used[(role, qtype)]
                return asked[len(asked) // 2], qtype
            # 문장이 떨어지면 같은 업무의 다른 유형 문장을 쓰되 유형도 그 문장에 맞춘다 —
            # 사실 확인 문장을 "판단"으로 기록하면 리포트의 질문 깊이가 거짓이 된다.
            for other_type, other in pool.items():
                if other:
                    question = other.pop(0)
                    used.setdefault((role, other_type), []).append(question)
                    return question, other_type
            for (used_role, used_type), asked in used.items():
                if used_role == role and asked:
                    return asked[len(asked) // 2], used_type
            return f"{by_role[role]['title']} 관련해서 한 번 더 여쭤봐도 될까요?", qtype
