...
# ── 청킹 헬퍼 (문장경계 존중 + 오버랩 + 업무명 prefix, 상위업무 포함) ──
...
def _parse_chapters_from_text(text: str) -> list[dict]:
    ...
    level(대분류=1, 소분류=2 이상)을 반환해 parent_id 계층을 저장 단계에서 구성한다
    (체크리스트 대분류·소분류 선택 UI, 사수 리포트 업무범위 점수가 이 값을 사용함).
    본문 없는 상위(그룹핑용) 헤더도 챕터로 유지한다 (guidelines 2-3 — content 없는
    상위 챕터 허용, 청킹·초안 대상에서만 제외) — 필터링하면 대분류가 통째로 사라짐.
    ...
    return [{"title": "전체 내용", "content": text, "level": 1, "fallback": True}]
...
def _parse_chapters_json(chapters_raw: str) -> list[dict]:
    ...
    직접입력 경로는 평면 구조 유지 (level=1 고정, B와 협의 완료 — 1차 빌드 범위 밖)."""
    try:
...
def _save_and_index_chapters(
    ...
    level==1(대분류)이 아닌 챕터는 가장 가까운 대분류를 parent_id로 잡는다
    (3단 이상도 전부 가장 가까운 대분류에 붙임 — B 프론트 UI가 대분류 직속
    자식만 소분류 목록에 표시하므로).
    본문 없는(그룹핑용) 챕터는 MySQL에는 저장하되, 청킹·임베딩·ChromaDB 저장은
    건너뛴다 (검색·초안 생성 대상에서 제외).
    """
    chapter_rows = []
    chapter_titles: dict[str, str] = {}  # chapter_id -> title (부모명 조회, 청크 접두사용)
    ...
    for row, ch in zip(chapter_rows, chapters):
        if not row.content:
            continue  # 그룹핑용 상위 챕터(본문 없음)는 청킹·임베딩 대상에서 제외

        if row.parent_id is not None:
            display_title = f"{chapter_titles.get(row.parent_id, '')} > {row.title}"
        else:
