...
- `/user/{user_id}` (DELETE, 신설): 회원 탈퇴. **토큰의 `user_id`가 경로의 `user_id`와 같아야 함** — 아니면 403 (3-9). **`DEMO_USER_IDS`에 있는 데모 계정은 본인이어도 탈퇴 불가(403 "데모 계정은 탈퇴할 수 없습니다")** — 심사위원 여러 명이 공유하는 계정이라 한 명이 누르면 그 회사 데모 전체가 막힌다 (신설, 9/16). 프론트도 데모 계정으로 로그인하면 탈퇴 버튼을 숨긴다 `users` 행만 삭제하는 1차 범위이며, 사수가 올린 문서/배정이나 신입의 채팅·체크리스트 기록에 대한 연쇄 삭제는 하지 않음 — 재배정 시 `chapter_id`가 고아로 남는 것(위 재배정 설명 참고)과 같은 성격의 accepted 1차 한계
