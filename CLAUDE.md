...
**Cost defense**: `/chat/ask`, `/checklist/draft`, `/report/generate` call OpenAI directly and are rate-limited both per-`user_id` and per-IP (guidelines §5-9) — per-user alone isn't enough since account creation has no email verification gate. Cap `max_tokens` on every OpenAI call. The demo newcomer accounts (§1-8) are shared by many simultaneous visitors, so `core/rate_limit.py` keys them on `user_id` + a per-visitor discriminator instead of `user_id` alone, via the `DEMO_USER_IDS` env var — `seed_demo.py` must insert the demo newcomers with the fixed ids `demo-newcomer-a`/`-b`/`-c` (not auto-generated UUIDs like a normal registration) so this always matches `.env.example`'s value with no manual copy-paste step (guidelines §6-6). The discriminator prefers the `X-Visitor-Id` header (a random id `api/client.js` generates once per browser into `localStorage`) over IP, falling back to IP only if the header is absent — plain IP isn't enough on its own since judges on the same venue WiFi would otherwise still collapse into one shared bucket.
